...
function file_get_html($url, $use_include_path = false, $context=null, $offset = -1, $maxLen=-1, $lowercase = true, $forceTagsClosed=true, $target_charset = DEFAULT_TARGET_CHARSET, $stripRN=true, $defaultBRText=DEFAULT_BR_TEXT)
{
    ...
    $dom = new parser(null, $lowercase = true, $forceTagsClosed = true, $target_charset = DEFAULT_TARGET_CHARSET, $defaultBRText = DEFAULT_BR_TEXT);
    // For sourceforge users: uncomment the next line and comment the retreive_url_contents line 2 lines down if it is not already done.
    $contents = file_get_contents($url, $use_include_path = false, $context = null, $offset = -1);
...
//    $contents = retrieve_url_contents($url);
    ...
    $dom->load($contents, $lowercase = true, $stripRN = true);
    return $dom;
}
...
{
    $dom = new parser(null, $lowercase, $forceTagsClosed, $target_charset, $defaultBRText);
    if (empty($str))
    {
        $dom->clear();
        return false;
...
class parser {
    ...
    function load_file() {
        ...
        // Per the parser repositiry this is a planned upgrade to the codebase.
    ...
    protected function prepare($str, $lowercase=true, $stripRN=true, $defaultBRText=DEFAULT_BR_TEXT) {
        ...
        $this->root = new parser_node($this);
        $this->root->tag = 'root';
    ...
    protected function read_tag() {
        ...
        if ($this->char==='/') {
            $this->char = (++$this->pos<$this->size) ? $this->doc[$this->pos] : null; // next
            // This represetns the change in the parser trunk from revision 180 to 181.
