...
class pager {
    ...
    function page_links ($inactive_page_tag, $pager_url_last) {
        ...
        for ($i = $loop_start; $i <= $loop_finish; $i++) {
            if ($i == $this -> current_page) {
                $this -> page_links .= '<a class="pages current"  href="javascript:;" '.$inactive_page_tag.'>'.$i.'</a>';
            }else{
                $this -> page_links .= '<a class="pages" href="'.$this -> url.$i.$pager_url_last.'">'.$i.'</a>';
            }
        }
    }
    ...
    function previous_page ($previous_page_text, $pager_url_last) {
        if ($this -> current_page > 1) {
            $this -> previous_page = '<a class="icon-prev" href="'.$this -> url.($this -> current_page - 1).$pager_url_last.'">'.$previous_page_text.'</a>';
        }
    }
    ...
    function next_page ($next_page_text, $pager_url_last) {
        if ($this -> current_page < $this -> total_pages) {
            $this -> next_page = '<a class="icon-next" href="'.$this -> url.($this -> current_page + 1).$pager_url_last.'">'.$next_page_text.'</a>';
        }
    }
